...
export const Erc20: React.FC = () => {
  ...
  return (
    <div className='flex flex-col md:flex-row md:justify-center justify-center items-stretch gap-3 w-full max-h-[80%]'>
      <Card className='p-6 flex flex-col w-full max-w-md backdrop-blur-sm bg-opacity-80 overflow-y-scroll mx-auto'>
        ...
        {erc20 && (
          <>
            ...
            <ReadView erc20={erc20} style={view !== 'read' ? hidden : undefined} />
            <WriteView erc20={erc20} style={view !== 'write' ? hidden : undefined} />
          </>
        )}
      </Card>
    </div>
...
// Style to preserve form layout when tab not selected
const hidden = { height: 0, overflow: 'hidden' }
